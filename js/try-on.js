...
(() => {
  ...
  async function loadWorld(){
    ...
    // Spawn avatar if user already selected one
  ...
  async function replaceAvatar(avatarUrl){
    ...
    try{
      ...
      alert("Avatar failed to load. Be sure the URL ends with .glb (e.g. https://models.readyplayer.me/XXXX.glb).");
    }
  }
  ...
  function buildOutfitBar(products){
    ...
    panel.addEventListener("click", async (e) => {
      ...
      alert(`Garment "${id}" coming soon.`); // placeholder hook
    });
  }
  ...
  function wireRackPickers(products){
    ...
    scene.meshes?.forEach?.(m => {
      ...
      m.actionManager.registerAction(new BABYLON.ExecuteCodeAction(
        ...
        async () => { alert(`Garment "${productId}" coming soon.`); }
      ));
    });
  }
...
// show avatar immediately (before world)
if (chosenAvatarUrl) { await replaceAvatar(chosenAvatarUrl); }

      // (Optional) show avatar before world — commented out for your flow
      // if (chosenAvatarUrl) { await replaceAvatar(chosenAvatarUrl); }
